...
- **Fullsend dispatch routing jobs** — the pre-script already drops checks
  named `dispatch / <Stage>`. If any remain, ignore them; they are expected
  no-ops, not `infra` failures
...
failed-job log excerpt as the value. Use these plus check metadata
(`conclusion`, `details_url`, `html_url`, `app_slug`) for diagnosis — do
not attempt to fetch logs via network calls. Check-run `output_title` /
`output_summary` / `output_text` are not in the context file.
